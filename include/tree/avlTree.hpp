...
wPtr<node_type> avlTree<T, __type, node_type>::insert(const T& k)
{
    ...
    this->fixUp(ptr); // Height is manged inside it.....
    return ptr;
}
...
T avlTree<T, __type, node_type>::remove(const wPtr<node_type>& ptr) noexcept
{
    if (!ptr.expired()) {
        ...
        this->fixUp(y); // Height is manged inside it.....
        return removed_data;
    }
    return T{};
}
...
void avlTree<T, __type, node_type>::balance(const wPtr<node_type>& ptr) noexcept
{
    if (!ptr.expired()) {
        auto x{ ptr.lock() };
        if (x != NIL) {
            size_t h_diff = ((x->left->h > x->right->h) ? (x->left->h - x->right->h) : (x->right->h - x->left->h));
            if (h_diff == 2) {
